...
def summary():
    ...
    # Determine pass or fail status and set the result message
    if percent >= PASSING_PERCENTAGE:
        result_message = f'<h2 style="color: green;">Result: Pass</h2>'
    else:
        result_message = f'<h2 style="color: red;">Result: Fail</h2>'

    html = f"""
        <!DOCTYPE html><html><head><title>Quiz Summary</title><style>
        body {{ font-family: sans-serif; margin: 20px; }}
        a {{ color: #007BFF; }}
        </style></head><body>
        <h1>Quiz Summary</h1>
        ...
        <p><i>(A score of {PASSING_PERCENTAGE}% or higher is required to pass.)</i></p><br>
